...
exports.test = async (req, res, next) => {
  ...
  console.log("server", aconfig.server);
  try {
    let connection = await sql.connect(aconfig);
    let dsTheLoai = await connection.request().query("select * from SINHVIEN");
    return res.status(200).send({ data: dsTheLoai.recordset });
  } catch (error) {
    console.log(error);
  }
...
const layKhuyenMai = async (res) => {
  try {
    ...
    for (let i = 0; i < data?.length; ++i) {
      if (
        currentDate < new Date(data[i].ngayKetThuc) &&
        currentDate >= new Date(data[i].ngayApDung)
      ) {
        kq = kq.map((k) => {
          let result;
    ...
    req.body.dssp.forEach((x) => {
      x.detail?.forEach(async (e) => {
        await connection
          ...
          .input("phanTramGiam", sql.Int, req.body.phanTramGiam)
          .query(
